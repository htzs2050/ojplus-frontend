...
const state = () => ({
    loginVisible: false,
    loginIndex: "1",
    accessToken: localStorage.getItem("accessToken") || "eyJhbGciOiJSUzI1NiIsInR5cCI6IkpXVCJ9.eyJleHAiOjI3MTM4NjQ1MTgsInR5cGUiOiJhY2Nlc3MiLCJ1c2VySUQiOjF9.zqgY0rQMoC-jhSqLEVWz5iy26uEa55GmNtL2VN6SiRPCxc5aYZ4NaEZxM3N9RNcSHwO89rhPrhfQ5J917H63yQ", //access改到-
...
const actions = {
    async refreshToken({ state, commit }: { state: any; commit: any }) {
        // 刷新令牌的动作，使用现有的refreshToken来获取新的令牌 async是异步函数
    ...
    async getToken({ commit }: { state: any; commit: any }, { account, password }: { account: any; password: any }) {
        // 根据用户名和密码获取令牌的动作
